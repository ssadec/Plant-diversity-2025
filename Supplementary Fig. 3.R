...
res <- read.csv('Stable17_Food&Cash.csv')
print(res)
...
Strat_labels <- c("Food_crop", "Cash_crop")
  ...
  --
    ...
    for (col in available_cols) {
      if (Strat_labels[j] %in% unique(dat[[col]])) {
        has_strat <- TRUE
        cat("in column", col, "value", Strat_labels[j], "\n")
        break
      }
    }
    
    if (!has_strat) {
      cat("warning: no data found", Strat_labels[j], "\n")
      next
    }
    ...
    cat("from res strat:", Strat_labels[j], "-", ylabs_short[i], "\n")
    cat("rows:", nrow(res_row), "\n")
...
if (length(all_plots) > 0) {
  png("Food&Cash_res.png", units = "cm", width = 40, height = 30, res = 300)
  ...
  dev.off()
  cat("pics saved as Food&Cash_res.png\n")
}
