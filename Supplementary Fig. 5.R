...
res <- read.csv(paste0(prefix2, "Stable17_NEs_G&S.csv"), na.strings = c("NA", ""))
...
for (j in seq_along(Strat)) {
  for (i in seq_along(ylabs)) {
    ...
    dat_sub <- subset(dat, NE_c == Strat[j])
...
png("NEs_G&S.png", units = "cm", width = 40, height = 30, res = 300)
...
for (i in seq_along(all_plots)) {
  ...
  if (i == 1) {
    text(5.5, 5.1, labels = "(315/73)", cex = 1.1, adj = 1)
  } else {
