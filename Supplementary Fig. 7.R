...
res <- read.csv('/Users/yusha/Desktop/check/Stable17_Study_design.csv')
print(res)
...
Strat_labels <- c("Natural", "Semi_natural", "Controlled")
...
for (j in seq_along(Strat)) {
  for (i in seq_along(ylabs)) {
    ...
    if (!file.exists(dat_path)) {
      cat("file does not exist:", dat_path, "\n")
      next
    }
    ...
    for (col in available_cols) {
      if (Strat_labels[j] %in% unique(dat[[col]])) {
        has_strat <- TRUE
        cat("in column", col, "data", Strat_labels[j], "\n")
        break
      }
    }
    
    if (!has_strat) {
      cat("warning: no data founded", Strat_labels[j], "\n")
      next
    }
    ...
    cat("n row after screening:", nrow(dat_sub), "\n")
    if (nrow(dat_sub) == 0) {
      cat("warning: after screening the data is 0\n")
      k <- k + 1
      next
    }
    ...
    cat("from res the results of strat:", Strat_labels[j], "-", ylabs_short[i], "\n")
    cat("n row:", nrow(res_row), "\n")
...
if (length(all_plots) > 0) {
  png("Study_design_res.png", units = "cm", width = 40, height = 30, res = 300)
  par(mai = c(1, 0.8, 0.15, 0.3), mfrow = c(3, 5), mgp = c(4, 1, 0))
  ...
  dev.off()
  cat(" pics saved as Study_design_res.png\n")
}
