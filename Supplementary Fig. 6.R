...
res <- read.csv(paste0(prefix2, "Stable17_NCP_Pattern.csv"), na.strings = c("NA", ""))
...
Strat <- c("Intercropping", "Cover_cropping","Sown_field_margins")
...
for (j in seq_along(Strat)) {
  for (i in seq_along(ylabs)) {
    ...
    dat_sub <- subset(dat, NCP_P == Strat[j])
    
    
    ###added_plant_species-->>LOGNCP_N
...
png("NCP_Pattern.png", units = "cm", width = 40, height = 30, res = 300)
...
for (i in seq_along(all_plots)) {
  ...
  if (pval_ok && richness_ok) {
    ...
    text(3.3, 6.0, labels = p_label, cex = 1.2, adj = 1)
  ...
  if (i == 11) {
    text(5.5, 5.1, labels = "(35/11)", cex = 1.1, adj = 1)
  } else {
