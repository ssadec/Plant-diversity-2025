...
for (i in Mn__) {
  ...
  for (j in datasets) {
    ...
    if (!("Mn__" %in% names(dat))) {
      warning("No Mn__ column in ", j, " -> skip")
      res <- rbind(res, data.frame(
        Mn__ = i, Category = j,
        Num_obs = NA, Num_studies = NA,
        Effect_size = NA, t_value = NA, P_value = NA,
...
# -------------------------------
colnames(res) <- c("Herbivore category", "Category", "Number of observations",
...
View(res)
write.csv(res,'/Users/yusha/Stable6.csv')
