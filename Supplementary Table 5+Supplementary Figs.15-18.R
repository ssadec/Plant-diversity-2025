...
for (i in CP_Ct) {
  ...
  for (j in datasets) {
    ...
    if (!("CP_Ct" %in% names(dat))) {
      warning("No CP_Ct column in ", j, " -> skip")
      res <- rbind(res, data.frame(
        CP_Ct = i, Category = j,
        Num_obs = NA, Num_studies = NA,
        Effect_size = NA, t_value = NA, P_value = NA,
...
# -------------------------------
colnames(res) <- c("Crop grown purpose", "Category", "Number of observations",
...
View(res)
write.csv(res,'/Users/yusha/Stable5.csv')
