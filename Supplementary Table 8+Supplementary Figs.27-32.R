...
NCP_P<- c("Intercropping", "Cover_cropping","Sown_field_margins")
k <- 27
...
for (i in NCP_P) {
  ...
  for (j in datasets) {
    ...
    if (!("NCP_P" %in% names(dat))) {
      warning("No NCP_P column in ", j, " -> skip")
      res <- rbind(res, data.frame(
        NCP_P = i, Category = j,
        Num_obs = NA, Num_studies = NA,
        Effect_size = NA, t_value = NA, P_value = NA,
    ...
    I2 <- tryCatch({
      ...
      if (!is.null(dat.fit$sigma2) && length(dat.fit$sigma2) >= 1) {
        ...
        tau2 <- dat.fit$tau2
        # if 'vi' exists in dat use mean(dat$vi) else mean(diag(V_tmp))
...
# -------------------------------
# -------------------------------
colnames(res) <- c("Crop grown pattern", "Category", "Number of observations",
...
View(res)
write.csv(res,'/Users/yusha/Stable8.csv')
