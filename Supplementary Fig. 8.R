...
res <- read.csv(paste0(prefix2, "Stable17_Zone.csv"), na.strings = c("NA", ""))
...
for (j in seq_along(Strat)) {
  for (i in seq_along(ylabs)) {
    ...
    dat_sub <- subset(dat, Zone == Strat[j])
    
    
    ###added_plant_species changed to LOGNCP_N
    ...
    R2   <- if (k <= nrow(res)) as.numeric(as.character(res$R2[k])) else NA
...
png("ZONE.png", units = "cm", width = 40, height = 30, res = 300)
...
for (i in seq_along(all_plots)) {
  ...
  if (pval_ok && richness_ok) {
    ...
    p_label <- if (!is.null(p$pval) && !is.na(p$pval)) {
      if (p$pval < 0.001) {
        "P < 0.001"
      } else {
        paste0("P = ", signif(p$pval, 3))
      }
    } else {
      NA
    }
    text(3.0, 6.0, labels = p_label, cex = 1.2, adj = 1)

    if (!is.null(p$R2) && !is.na(p$R2)) {
      text(5.5, 6.0, labels = paste("R² =", round(p$R2, 4)), cex = 1.2, adj = 1)
    }
  }
  ...
  if (i == 2) {
    text(5.5, 5.1, labels = "(78/25)", cex = 1.1, adj = 1)
  } else if (i == 9) {
    text(5.5, 5.1, labels = "(256/36)", cex = 1.1, adj = 1)
  } else {
