...
Std_<- c("Natural","Semi_natural","Controlled")
k <- 33
...
for (i in Std_) {
  ...
  for (j in datasets) {
    ...
    if (!("Std_" %in% names(dat))) {
      warning("No Std_ column in ", j, " -> skip")
      res <- rbind(res, data.frame(
        Std_ = i, Category = j,
        Num_obs = NA, Num_studies = NA,
        Effect_size = NA, t_value = NA, P_value = NA,
...
# -------------------------------
colnames(res) <- c("Study design", "Category", "Number of observations",
...
View(res)
write.csv(res,'/Users/yusha/Stable9.csv')
